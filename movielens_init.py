    ...
    def initialize(self):      
        ...
        self.cursor.execute("""
                CREATE VIRTUAL TABLE feature_embedding_test using vectorlite (
                    ...
                    hnsw(max_elements=150000),'index_file_test.bin'
                );
                """)
...
class Validator:
    ...
    def validate(self):
        ...
        for row in df_test.itertuples(index=True, name="DataRow"):
            ...
            #         "SELECT rowid FROM feature_embedding_test order by vector_distance(?, embedding, 'cosine') asc limit ?", (vector, 1)
            # ).fetchall()
            print(vector)
            try:
                result = self.cursor.execute(
                        "SELECT rowid FROM feature_embedding_test WHERE knn_search(embedding, knn_param(?, ?))", (vector, 1)
                ).fetchall()
            except sqlite3.Error as e:
                print(e)
